-- Produto com apenas uma cor disponível (ex.: AirPods branco): cliente não escolhe 2 cores.

ALTER TABLE public.landing_products
  ADD COLUMN IF NOT EXISTS single_color boolean NOT NULL DEFAULT false;
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(NULLIF(trim(elem->>'category'), ''), 'Celular'),
    COALESCE(NULLIF(trim(elem->>'brand'), ''), 'Apple'),
    COALESCE((elem->>'is_on_sale')::boolean, false),
    COALESCE(trim(elem->>'color'), ''),
    COALESCE((elem->>'single_color')::boolean, false),
    ...
    END,
    CASE
      WHEN jsonb_typeof(elem->'model_options') = 'array' THEN elem->'model_options'
      ELSE '[]'::jsonb
    END,
